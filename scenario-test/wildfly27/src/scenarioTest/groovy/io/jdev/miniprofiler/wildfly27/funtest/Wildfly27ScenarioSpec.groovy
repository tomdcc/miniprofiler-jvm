...
class Wildfly27ScenarioSpec extends Specification {
    ...
    void "profiling data returned for request"() {
        ...
        def resultResponse = httpGet("${baseUrl}miniprofiler/results?id=${ids[0]}", [Accept: 'application/json'])
        ...
        sqlTimings.size() == 1
        sqlTimings[0].CommandString =~ /(?i)select\s+p1_0\.id,\s*p1_0\.firstName,\s*p1_0\.lastName\s+from\s+Person\s+p1_0/
    }
}
