...
class JakartaServletMiniprofilerFunctionalSpec extends GebReportingSpec {
    ...
    void "can see miniprofiler"() {
        ...
        timings.size() == 1
        timings[0].label == '/jakarta-servlet/'
        ...
        queries[1].step == '/jakarta-servlet/'
        ...
        withWindow({ MiniProfilerSingleResultPage.matches(driver) }, page: MiniProfilerSingleResultPage) {
            driver.title ==~ /\/jakarta-servlet\/ .* = Profiling Results/
            waitFor {
                page.items.size() == 2
            }
        }
